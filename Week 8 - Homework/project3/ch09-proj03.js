    ...
    child.addEventListener("click", function() {
        ...
        for (var i = 0; i < portfolio.length; i++) {
            ...
            table.innerHTML += `<h3 data-symbl="${stock_symbol}">${stock_symbol}</h3>
                                <h3>${stock_owned}<h3>
                                <button id="btnView" data-symb="${stock_symbol}">View</button>`;
            ...
            document.getElementById("btnView").addEventListener("click", function() {
                fetch('./stocks-formatted.json').then(results=>results.json()).then(data=>{
                    ...
                    document.getElementById("stockName").innerHTML=stock_name;
                    document.getElementById("stockSector").innerHTML=stock.sector;
                    document.getElementById("stockIndustry").innerHTML=stock.subIndustry;
                    document.getElementById("stockAddress").innerHTML=stock.address;
