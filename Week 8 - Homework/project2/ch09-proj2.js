document.addEventListener("DOMContentLoaded", function() {
    ...
    const paintings = JSON.parse(paintingsJSON);
    ...
    paintingsList.addEventListener("click", function(event) {
      if (event.target.tagName === "IMG") {
        const painting = paintings.find(function(painting) {
        ...
        while (figureTag.children.length > 2) {
          figureTag.removeChild(figureTag.lastChild);
        }

        painting.features.forEach(function(feature) {
            const box = document.createElement("div");
            
            feature.x=feature.upperLeft[0];
            feature.y=feature.upperLeft[1];
            feature.width=feature.lowerRight[0]-feature.upperLeft[0];
            feature.height=feature.lowerRight[1]-feature.upperLeft[1];
            ...
            figureTag.appendChild(box);
            console.log(feature.description)
            });



        /* paintings.features.forEach(function(feature) {
          const box = document.createElement("div1");
          box.classList.add("box");
          box.style.position = "absolute";
          box.style.left = feature.x + "px";
          box.style.top = feature.y + "px";
